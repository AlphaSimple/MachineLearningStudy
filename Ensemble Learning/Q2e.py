...
N_SAMPLE = 1000
N_TREES = 500
K = 4  # Feature subset size for random forest
...
def main():
    ...
    forest_preds = np.zeros((N_RUNS, n_test))

    print(f'Running bias-variance experiment for random forest (k={K})...')
    for run in tqdm(range(N_RUNS), desc='Runs'):
        ...
        # Single random tree: first tree in each run
        single_tree_preds[run, :] = np.array([1 if p == 'yes' or p == 1 else -1 for p in all_trees[0].predict(X_test)])
        # Random forest: majority vote of all 500 trees
        all_tree_preds = np.array([[1 if p == 'yes' or p == 1 else -1 for p in tree.predict(X_test)] for tree in all_trees])
        forest_pred = np.sign(np.sum(all_tree_preds, axis=0))
        forest_pred[forest_pred == 0] = 1  # break ties as +1
        forest_preds[run, :] = forest_pred

    # Bias-variance for single random tree
    ...
    # Bias-variance for random forest
    forest_means = np.mean(forest_preds, axis=0)
    forest_bias2 = np.mean((forest_means - y_test) ** 2)
    forest_var = np.mean(np.var(forest_preds, axis=0, ddof=1))
    forest_sqerr = forest_bias2 + forest_var

    print('Single random tree:   Bias^2 = {:.4f}, Variance = {:.4f}, Squared Error = {:.4f}'.format(single_bias2, single_var, single_sqerr))
    print('Random forest:        Bias^2 = {:.4f}, Variance = {:.4f}, Squared Error = {:.4f}'.format(forest_bias2, forest_var, forest_sqerr))
