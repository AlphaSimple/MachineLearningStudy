...
from gradient_descent import stochastic_gradient_descent, compute_cost
...
def plot_cost_history(cost_history, learning_rate):
    ...
    plt.xlabel('Update')
    plt.ylabel('Cost')
    plt.title('Cost Function Value vs. Update')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('cost_history_plot_sgd.png')
    plt.show() 
...
if __name__ == "__main__":
    ...
    while True:
        ...
        w, b, cost_history, updates = stochastic_gradient_descent(X_train, y_train, r, tol=tol, max_iter=max_iter)
        if len(cost_history) < max_iter:  # Converged
            ...
            print(f"Converged in {updates} updates with r={r}")
            break
        else:
            print(f"Did not converge in {len(cost_history)} updates with r={r}")
    ...
    print(f"Number of updates: {best_updates}")
